...
//grid density using the PSC kernel (instead of CIC which is used below)
void GridParticles_PSC(int nx, int ny, int nz, std::vector<float> &Pos, float Boxlength, unsigned nPart, float m,  fftw_real *data , float subbox, int *ourbox){
    ...
    for (i = 0; i < 3; i++){ if (ourbox[i] > subbox || ourbox[i] < 1){error (-1,0,"Error with ourbox."); } }

    for( unsigned i=0; i<nPartThis; ++i ){

        x = Pos[3*i+0]; //all positions here are in units of grid cells, i.e. grid-spacing == 1
        y = Pos[3*i+1];
        z = Pos[3*i+2];
        ...
        if (x < nx && y < ny && z < nz && x > 0. && y > 0. && z > 0.){
          ++icount; //this is not used anywhere else
        
          ix = (unsigned)x; //rounding towards 0 (same as floor for positive values, which is assured by above if)
          iy = (unsigned)y;
          iz = (unsigned)z;

          dx = (x-((float)ix)); //difference from left grid point
          dy = (y-((float)iy));
          dz = (z-((float)iz));

          ix %= nx; //making sure that everything stays within the grid (probably unneccessary)
          iy %= ny;
          iz %= nz;

          tx = 1.0f-dx; //difference from right grid point
          ty = 1.0f-dy;
          tz = 1.0f-dz;

          ix1 = (ix+1)%nx; //move one grid point to the right
          iy1 = (iy+1)%ny;
          iz1 = (iz+1)%nz;

          ix2 = (ix+2)%nx; //move two grid point to the right
          iy2 = (iy+2)%ny;
          iz2 = (iz+2)%nz;
...
//  nz /= 2.;  // tag+
  ...
  // FFT norm brings 1/N^3, conversion to physical units L^3/N^3, definition of powerspectrum brings (2pi)^-3
...
int main( int argc, char *argv[])
{
  ...
  //std::cout << " Please specify grid resolution for FFT: ";
  //std::cin >> ngrid;
  
  int ourbox[3];
  ourbox[0] = atoi(argv[5]);
...
//	std::cerr << "Read " << nread << " of " << numpartDM << ".\n";
  ...
  //omputePowerSpectrum( argv[3], nBins, nx, ny, nz, Boxlength, numpartDM, data );
