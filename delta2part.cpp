...
double F(int q1, int q2, int q3, int p1, int p2, int p3){
		...
		double eps=0.0000001,value=0.,modq,modp,qp, moddiff;
...
double H(int q1, int q2, int q3, int p1, int p2, int p3){
	...
	double eps=0.0000001,value=0.,modp,qp;
...
int main(int argc, char *argv[]){
	...
	double *arr=(double*)calloc(res*res*res,sizeof(double));
	...
	// H5Dread(datasetid,H5T_NATIVE_DOUBLE, memspacebuf,filespacebuf,H5P_DEFAULT, arr);
	...
	for(id=idstart;id<idstopn;id++){
		...
		for(idq1=0;idq1<res*res*res;idq1++){
			...
			v2[idk].re+=(double)(5.*(h1*A+h2*B)+4.*f*C)/14.;

			A=d2re*d1im+d1re*d2im;
			B=d1re*d2im+d1im*d2re;
			C=d1re*d2im+d1im*d2re;

			v2[idk].im+=(double)(5.*(h1*A+h2*B)+4.*f*C)/14.;	
